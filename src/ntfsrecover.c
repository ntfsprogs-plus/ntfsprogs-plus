...
u32 get_extra_offset(const LOG_RECORD *logr)
{
	...
	roffset = get_redo_offset(logr) + le16_to_cpu(logr->redo_length);
	uoffset = get_undo_offset(logr) + le16_to_cpu(logr->undo_length);
		...
		if (likely && logr->redo_length && logr->undo_length) {
			...
			if (logr->undo_offset == logr->redo_offset) {
				...
				if (le16_to_cpu(logr->redo_offset) <
					...
					if ((((le16_to_cpu(logr->redo_offset) +
						le16_to_cpu(logr->redo_length) - 1) | 7) + 1) !=
							le16_to_cpu(logr->undo_offset))
						likely = FALSE;
				} else {
					/* redo expected just after undo */
					if ((((le16_to_cpu(logr->undo_offset) +
						le16_to_cpu(logr->undo_length) - 1) | 7) + 1) !=
...
static u16 searchlikely(const struct BUFFER *buf)
{
	...
	if (!likelyop(logr)) {
		do {
			k += 8;
			logr = (const LOG_RECORD*)&data[k];
		} while ((k <= (blocksz - LOG_RECORD_HEAD_SZ)) && !likelyop(logr));
	...
	if (k && ((blocksz - k) >= LOG_RECORD_HEAD_SZ)) {
		...
		if (!logr->client_data_length) {
			...
			if ((k <= blkheadsz) &&
				...
				if (optv && ((blocksz - k) < LOG_RECORD_HEAD_SZ)) {
		...
		if (prevblk > (log_major < 2 ? BASEBLKS : BASEBLKS2))
			...
			if (prevblk == (log_major < 2 ? BASEBLKS : BASEBLKS2))
				...
				if (log_major < 2)
					...
					prevblk = (sle64_to_cpu(rph->copy.last_lsn) &
							offset_mask) >>
						(blockbits - 3);
				/*
	...
	do {
		...
		for (action=lastaction; action; action=action->prev) {
			...
			if ((logr->redo_operation ==
					...
					!(action->flags & ACTION_TO_REDO) && !id) {
			...
			committed = ((s64)(sle64_to_cpu(logr->this_lsn) -
						committed_lsn)) <= 0;
			if (!logr->transaction_id && committed)
				action->flags |= ACTION_TO_REDO;
			if (id && (logr->transaction_id == id) && committed) {
				action->flags |= ACTION_TO_REDO;
				more = TRUE;
			}
		}
		...
		const struct BUFFER *nextbuf)
{
	...
	if (nextbuf && (blk >= BASEBLKS)) {
		...
		if ((space >= LOG_RECORD_HEAD_SZ) && (size > space)) {
			...
			if (size <= (space + nextspace)) {
				...
				next = (size - space) % nextspace + blkheadsz;
		...
		while (!stop) {
			...
			if ((size < MINRECSIZE) ||
				...
				if (((u32)(k + size) <= blocksz) &&
					...
					showlogr(ctx, k, logr);
					if (!logr->client_data_length) {
						printf("** Bad client_data_length\n");
						stop = TRUE;
					}
					k += size;
					if ((blocksz - k) < LOG_RECORD_HEAD_SZ) {
						k = nextbuf->headsz;
						stop = TRUE;
					}
				} else {
					k = overlapshow(ctx, k, blk, buf, nextbuf);
					stop = TRUE;
				}
			}
		}
	...
	for (blk=RSTBLKS; 2*blk<(RSTBLKS+BASEBLKS2); blk++) {
		...
		if (buf && (buf->block.record.magic == magic_RCRD)) {
			last_lsn2 = le64_to_cpu(
					buf->block.record.copy.last_lsn);
			if (!final_lsn || ((s64)(last_lsn2 - final_lsn) > 0))
				final_lsn = last_lsn2;
		}
		...
		if (last_lsn1 || last_lsn2) {
			if (!last_lsn2 || ((s64)(last_lsn1 - last_lsn2) > 0)) {
				last_lsn12 = last_lsn1;
				latest_blk = blk;
			}
			if (!last_lsn1 || ((s64)(last_lsn1 - last_lsn2) <= 0)) {
		...
		if (last_lsn12) {
			...
			if (buf && (buf->block.record.magic == magic_RCRD)) {
				last_lsn = le64_to_cpu(
						buf->block.record.copy.last_lsn);
				if (!final_lsn || ((s64)(last_lsn - final_lsn) > 0))
					final_lsn = last_lsn;
			}
		}
...
static int walk(CONTEXT *ctx)
{
	...
	if (log_major >= 2)
		blk = finalblk;
	if (!err && (optb || optp || optu || (opts && dirty))) {
		playedactions = 0;
		ctx->firstaction = (struct ACTION_RECORD*)NULL;
		ctx->lastaction = (struct ACTION_RECORD*)NULL;
		if (log_major < 2) {
...
static BOOL checkstructs(void)
{
	...
	if (sizeof(RECORD_PAGE_HEADER) != 40) {
		fprintf(stderr, "* error : bad sizeof(RECORD_PAGE_HEADER) %d\n",
				(int)sizeof(RECORD_PAGE_HEADER));
		ok = FALSE;
	}
	if (sizeof(LOG_RECORD) != 88) {
		fprintf(stderr, "* error : bad sizeof(LOG_RECORD) %d\n",
				(int)sizeof(LOG_RECORD));
		ok = FALSE;
	}
	if (sizeof(RESTART_PAGE_HEADER) != 32) {
		fprintf(stderr, "* error : bad sizeof(RESTART_PAGE_HEADER) %d\n",
				(int)sizeof(RESTART_PAGE_HEADER));
		ok = FALSE;
	}
	if (sizeof(RESTART_AREA) != 48) {
		fprintf(stderr, "* error : bad sizeof(RESTART_AREA) %d\n",
				(int)sizeof(RESTART_AREA));
		ok = FALSE;
	}
	if (sizeof(ATTR_OLD) != 44) {
		fprintf(stderr, "* error : bad sizeof(ATTR_OLD) %d\n",
				(int)sizeof(ATTR_OLD));
		ok = FALSE;
	}
	if (sizeof(ATTR_NEW) != 40) {
		fprintf(stderr, "* error : bad sizeof(ATTR_NEW) %d\n",
				(int)sizeof(ATTR_NEW));
		ok = FALSE;
	}
	if (LastAction != 38) {
		fprintf(stderr, "* error : bad action list, %d actions\n",
				(int)LastAction);
		ok = FALSE;
	}
	return (ok);
...
int main(int argc, char *argv[])
{
	...
	err = 1;
	if (checkstructs() && getoptions(argc,argv)) {
		if (optV || opth) {
			err = 0;
		} else {
			redocount = 0;
