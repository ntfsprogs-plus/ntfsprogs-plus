...
static int parse_options(int argc, char **argv)
{
	...
	if (!help && !ver) {
		...
		if (opt.bytes && (opt.expand || opt.info || opt.infombonly)) {
...
static void delayed_updates(ntfs_resize_t *resize)
{
	...
	if (opt.ro_flag && resize->delayed_runlists &&
	...
	if (ok) {
		...
		if (!expand->mft_size || !expand->boot_size) {
			...
			sectors_needed = (expand->boot_size + expand->mft_size +
					expand->bitmap_allocated) >>
				vol->sector_size_bits;
			if (!opt.info && (sectors_needed >= expand->sector_increment)) {
	...
	if (buf) {
		...
		while (!res && (written < boot_cnt)) {
			...
			if (!read_all(vol->dev, buf, vol->cluster_size)) {
				...
				if (!opt.ro_flag && write_all(vol->dev, buf, vol->cluster_size)) {
		...
		s64 inum, s64 jnum)
{
	...
	if (!res) {
		...
		if (!opt.ro_flag && (ntfs_mst_pwrite(vol->dev, pos, 1,
						vol->mft_record_size, mrec) != 1)) {
